...
Terminal::Terminal(Polygone* polygone, int number)
{
	...
	if(polygone->points3D.size()>0) //Utilise t on les coordonn�es 3D ?
	{
	...
	if(polygone->points3D.size()==4) //On consid�re que c'est un rectangle si 4 sommets
...
{
	return n==this; //Un seul terminal cr�� donc on peut comparer les pointeurs
} 
