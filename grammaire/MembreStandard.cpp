...
set<Noeud*> MembreStandard::getAffectations(Parser* p,Noeud** affectation, int N)
{
		...
		for(vector<ConditionAdj*>::iterator it = parent->condAdj.begin();it!=parent->condAdj.end();it++)
		{
			...
			else if(cadj->j==this->position && affectation[cadj->i]!=NULL)
			{
				...
				for(set<Polygone*>::iterator it2 = adj.begin();it2!=adj.end();it2++)
				{
					...
					//cout<<"Nombre de noeuds adjacents � "<<(*it2)->number<<" : "<<parent->table_adj_i[cadj][*it2].size()<<endl;
				}
				set<Noeud*> new_set;
				set_intersection(resultats.begin(),resultats.end(),adj_elem.begin(),adj_elem.end(),std::inserter(new_set, new_set.begin()));
				resultats = new_set;
	...
	if(parent->condAdj_ext.size()>0){
		//cout<<"Taille des tables de hachage :"<<parent->table_adj_i[parent->condAdj[0]].size()<<endl;
		for(vector<ConditionAdj_Ext*>::iterator it = parent->condAdj_ext.begin();it!=parent->condAdj_ext.end();it++)
		{
			ConditionAdj_Ext* cadj = *it;
			if(cadj->i==this->position && affectation[cadj->j]!=NULL)
			{
				//cout<<"Prise en comppte d'une condition d'adjacence"<<endl;
				set<Polygone*> adj = p->adj_ext[affectation[cadj->j]->getAttribut(cadj->att_j)->polygoneValue()];
				//cout<<"Nombre d'�l�ments adjacents :"<<adj.size()<<endl;
				set<Noeud*> adj_elem;
				for(set<Polygone*>::iterator it2 = adj.begin();it2!=adj.end();it2++)
					adj_elem.insert(parent->table_adj_ext_i[cadj][*it2].begin(),parent->table_adj_ext_i[cadj][*it2].end());
				set<Noeud*> new_set;
				set_intersection(resultats.begin(),resultats.end(),adj_elem.begin(),adj_elem.end(),std::inserter(new_set, new_set.begin()));
				resultats = new_set;
			...
			{
				//cout<<"Prise en comppte d'une condition d'adjacence pour j"<<endl;
				set<Polygone*> adj = p->adj_ext[affectation[cadj->i]->getAttribut(cadj->att_i)->polygoneValue()];
				//cout<<"Nombre d'�l�ments adjacents :"<<adj.size()<<endl;
				/*for(set<Polygone*>::iterator it= adj.begin();it!=adj.end();it++)
					cout<<(*it)->number<<" ";*/
				//cout<<endl;
				...
				for(set<Polygone*>::iterator it2 = adj.begin();it2!=adj.end();it2++)
				{
					adj_elem.insert(parent->table_adj_ext_j[cadj][*it2].begin(),parent->table_adj_ext_j[cadj][*it2].end());
					//cout<<"Nombre de noeuds adjacents � "<<(*it2)->number<<" : "<<parent->table_adj_i[cadj][*it2].size()<<endl;
				}
				set<Noeud*> new_set;
				set_intersection(resultats.begin(),resultats.end(),adj_elem.begin(),adj_elem.end(),std::inserter(new_set, new_set.begin()));
	...
	if(parent->condAdj_int.size()>0){
			//cout<<"Taille des tables de hachage :"<<parent->table_adj_i[parent->condAdj[0]].size()<<endl;
			for(vector<ConditionAdj_Int*>::iterator it = parent->condAdj_int.begin();it!=parent->condAdj_int.end();it++)
			{
				ConditionAdj_Int* cadj = *it;
				if(cadj->i==this->position && affectation[cadj->j]!=NULL)
				{
					//cout<<"Prise en comppte d'une condition d'adjacence"<<endl;
					set<Polygone*> adj = p->adj_int[affectation[cadj->j]->getAttribut(cadj->att_j)->polygoneValue()];
					//cout<<"Nombre d'�l�ments adjacents :"<<adj.size()<<endl;
					set<Noeud*> adj_elem;
					for(set<Polygone*>::iterator it2 = adj.begin();it2!=adj.end();it2++)
						adj_elem.insert(parent->table_adj_int_i[cadj][*it2].begin(),parent->table_adj_int_i[cadj][*it2].end());
					set<Noeud*> new_set;
					set_intersection(resultats.begin(),resultats.end(),adj_elem.begin(),adj_elem.end(),std::inserter(new_set, new_set.begin()));
					resultats = new_set;
				...
				{
					//cout<<"Prise en comppte d'une condition d'adjacence pour j"<<endl;
					set<Polygone*> adj = p->adj_int[affectation[cadj->i]->getAttribut(cadj->att_i)->polygoneValue()];
					//cout<<"Nombre d'�l�ments adjacents :"<<adj.size()<<endl;
					/*for(set<Polygone*>::iterator it= adj.begin();it!=adj.end();it++)
						cout<<(*it)->number<<" ";*/
					//cout<<endl;
					...
					for(set<Polygone*>::iterator it2 = adj.begin();it2!=adj.end();it2++)
					{
						adj_elem.insert(parent->table_adj_int_j[cadj][*it2].begin(),parent->table_adj_int_j[cadj][*it2].end());
						//cout<<"Nombre de noeuds adjacents � "<<(*it2)->number<<" : "<<parent->table_adj_i[cadj][*it2].size()<<endl;
					}
					set<Noeud*> new_set;
					set_intersection(resultats.begin(),resultats.end(),adj_elem.begin(),adj_elem.end(),std::inserter(new_set, new_set.begin()));
	...
	/* Le == marche plus, il faut modifier la classe Attribut pour red�finir l'�galit� (ou plutot l'in�galit�*/
	...
	for(int i=0;i<N;i++)
	{
		...
		for(set<Noeud*>::iterator it_exclu = exclu.begin();it_exclu!=exclu.end();it_exclu++)
		{
			...
			//cout<<"On supprime "<<(*it_exclu)->nom_parser<<" des r�sultats car "<<parent->membresDroits[i]->name<<" vaut "<<affectation[i]->nom_parser<<endl;
		}
